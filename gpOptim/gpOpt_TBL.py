...
sigma_d = 0.001       #sdev of the white noise in the measured data   
whichOptim = 'min'  #find 'max' or 'min' of f(x)?
kernelType = 'Matern52'  #'RBF', 'Matern52'
#admissible range of parameters
#qBound = [[41,47], [42,51], [44,53], [47,55], [47,56], [46,55], [44,54], [41,50]]
qBound = [[42,44], [45,47], [48,50], [50.5,52.5], [51,53.5], [49.5,51.5], [47,49], [44,46]] 
...
tol_d = 0*qMaxDist        #minimum distace between two consequtive samples x to keep the code running
...
def my_convergence_plot(xList, yList, figDir, figName):
    ...
    fig = plt.gcf()
    DPI = fig.get_dpi()
    fig.set_size_inches(900/float(DPI), 1800/float(DPI))
...
def gpOpt2d_postProc(xGP, yGP, bounds, plotOpts, final=False, kernelType_=kernelType):
    ...
    if nPar < 2:
        ...
        if nPar == 2:
            ...
            # for nPar >= 3: use a (nPar-1) x (nPar-1) grid
            gridSide = nPar - 1
            # build all unique pairs (i < j) and map them to a lower-triangular grid
            for j in range(1, nPar):         # second index
                for i in range(0, j):        # first index, i < j
                    parID.append([i, j])
                    # row = j-1, col = i  (0-based)
                    loc_idx = (j - 1) * gridSide + i + 1   # 1-based subplot index
                    loc.append(loc_idx)

    # --- Figure size based on nPar, ratio 8:5 ---
    scaleFactor = nPar / 1.0      # e.g., nPar=8 → scale=4 → (32,20)
    figWidth  = 8 * scaleFactor
    figHeight = 5 * scaleFactor
    ...
    for i in range(len(parID)):  #param-pair loop
        ...
        if nPar == 2:
            ...
            gridSide = nPar - 1
            ax = fig.add_subplot(gridSide, gridSide, loc[i])
    ...
    plt.tight_layout()  # let matplotlib adjust spacing safely
    plt.savefig(figSave + '.pdf')  # <-- NO tight bbox
