...
def add_handler():
    ...
    fh = logging.FileHandler("driver.log")  # Creates real file
...
U_infty, delta99_in, Nx, Ny, Nz, tEnd, Lx, Ly, nProcessors = \
    1, 0.05, int(500), int(218), int(1), int(200), 50, 2, 20
...
PATH2LOG = current_dir + "/OFcase/log"
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(PATH2BUP):
       #os.mkdir(PATH2BUP)          # Linux system
       os.makedirs(PATH2BUP, exist_ok=True)     # Windows system
    if not os.path.isdir(PATH2DATA):
       #os.mkdir(PATH2DATA)          # Linux system
       os.makedirs(PATH2DATA, exist_ok=True)     # Windows system
    if not os.path.isdir(PATH2FIGS):
       #os.mkdir(PATH2FIGS)          # Linux system
       os.makedirs(PATH2FIGS, exist_ok=True)     # Windows system
    ...
    for i in range(iStart, iEnd + 1):
        ...
        subprocess.call("foamListTimes -rm", shell=True) # delete time directories (except 0) This is not working
        subprocess.call("rm -rf %d" % tEnd, shell=True) # Alternative
        ...
        #subprocess.check_call("mpirun -np %d simpleFoam -parallel > log &" % (nProcessors), shell=True) # for run in workstation (not cluster)
...
#        os.chdir(current_dir)
    ...
    subprocess.check_call("cp %s %s/" % (PATH2LOG, PATH2BUP), shell=True)
    subprocess.check_call("cp driver.log %s/" % (PATH2BUP), shell=True)    
