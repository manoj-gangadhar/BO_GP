...
def load_grid(path2run,casename,Nx,Ny,Nz):
    ...
    try:
        ...
        logger.error("coundn't load 0/C")
        sys.exit(1)
    ...
    try:
        ...
        logger.error("coundn't load polyMesh/points")
        sys.exit(1)
...
def bl_calc(Nx,Ny,Nz,xc,yc,x,y,U_eIn,nu,U,V,p,nut,k,omega,tau_w):
    ...
    for i in range(Nx+1):
        if i == 0:
            ...
            yp[:,Nx] = y[:,Nx]*u_tau[-1]/nu
            xp[Nx] = x[Nx]*u_tau[-1]/nu
        else:
            yp[:,i] = y[:,i]*0.5*(u_tau[i-1]+u_tau[i])/nu
            xp[i] = x[i]*0.5*(u_tau[i-1]+u_tau[i])/nu
...
#         U_tmp = U[:,i]/U_max[i]
#         counter = 0 # avoid infinity loop
#         index=np.where(U[:,i]==U_max[i])[0][0]
#         while True:
#             while True:
#                 try:
...
def distance(x,y,a): # line y=f(x) and [a0,a1]
    if np.size(x)!=np.size(y) or np.size(a)!=2:
        logger.error('distance func.: check input')
...
#    fig.add_axes(cax)
    ...
    ax.set_xlim(0,np.max(xc_delta))
    ax.set_ylim(0,np.max(yc_delta))
...
def save_figure(figname,casename,formatname):
    ...
    if 'pdf' in formatname:
        logger.info('save ./figures/%s_%s.pdf' % (figname,casename))
